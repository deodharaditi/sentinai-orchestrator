...
const mdComponents = {
  p:      ({ children }: C) => <p style={{ color: "#3fb950", margin: "0 0 6px 0", lineHeight: "1.6" }}>{children}</p>,
  h1:     ({ children }: C) => <p style={{ color: "#e6edf3", fontWeight: 700, fontSize: "13px", margin: "10px 0 4px" }}>{children}</p>,
  h2:     ({ children }: C) => <p style={{ color: "#e6edf3", fontWeight: 700, fontSize: "12px", margin: "8px 0 3px" }}>{children}</p>,
  h3:     ({ children }: C) => <p style={{ color: "#58a6ff", fontWeight: 600, margin: "6px 0 3px" }}>{children}</p>,
  strong: ({ children }: C) => <strong style={{ color: "#e6edf3" }}>{children}</strong>,
  em:     ({ children }: C) => <em style={{ color: "#8b949e" }}>{children}</em>,
  code:   ({ children }: C) => (
    <code style={{ backgroundColor: "#0d1117", color: "#58a6ff", padding: "1px 5px", borderRadius: "3px", fontFamily: "monospace" }}>
      {children}
    </code>
  ),
  ul: ({ children }: C) => <ul style={{ margin: "4px 0", paddingLeft: "18px" }}>{children}</ul>,
  ol: ({ children }: C) => <ol style={{ margin: "4px 0", paddingLeft: "18px" }}>{children}</ol>,
  li: ({ children }: C) => <li style={{ color: "#3fb950", margin: "2px 0" }}>{children}</li>,
